...
const paths = [
  ...
  "/v1/listings/DWE-3002501/similar",
  "/v1/favourites",
...
for (const collection of ["listings", "rentals", "projects"]) {
  for (const query of [
    ...
    "sort_by=" +
      (collection === "projects" ? "price_min" : "price") +
      "&order=asc",
    "sort_by=" +
      (collection === "projects" ? "price_min" : "price") +
      "&order=desc",
  ])
    paths.push(`/v1/${collection}?${query}`);
}
for (const q of [
  "bhk=2",
  "bedroom=2",
  ...
  "property_type=villa",
  "project_id=P30001",
  "sort_by=carpet_area&order=desc",
  "sort_by=posted_at&order=desc",
  "sort_by=bedroom&order=desc",
])
  paths.push("/v1/listings?" + q);
paths.push("/v1/rentals?bhk=2", "/v1/projects?project_status=completed");
const probes = [];
for (const path of paths) {
  const result = await request(path);
  probes.push({ path, ...result });
  const b = result.body;
  console.log(
    path,
    result.status,
    b.results
      ? JSON.stringify({
          ...Object.fromEntries(
            Object.entries(b).filter(([k]) => k !== "results"),
          ),
          sample: b.results
            .slice(0, 3)
            .map((x) => ({
