...
export async function request(path, options = {}) {
  ...
  const response = await fetch(base + path, {
    ...options,
    headers: { 'X-API-Key': process.env.IVY_API_KEY, Authorization: `Bearer ${session.access_token}`, 'Content-Type': 'application/json', ...options.headers },
    ...(options.body ? { body: JSON.stringify(options.body) } : {}),
  });
  const body = await response.json();
  if (response.status === 401 && !path.startsWith('/auth/')) {
    const refreshed = await request('/auth/refresh', { method: 'POST', body: { refresh_token: session.refresh_token } });
