...
for (const collection of ["listings", "rentals", "projects"]) {
  ...
  while (true) {
    const { status, body } = await request(
      `/v1/${collection}?limit=200&offset=${offset}`,
    );
    if (status !== 200) throw new Error(JSON.stringify(body));
    const { results, ...meta } = body;
    if (meta.offset !== offset || meta.count !== results.length)
      throw new Error("Pagination contract changed");
